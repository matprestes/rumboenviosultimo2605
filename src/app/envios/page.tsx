...
export default function EnviosPage() {
  ...
  const [fechaDesdeFilter, setFechaDesdeFilter] = React.useState<Date | undefined>(
    searchParams.get('desde') ? parseISO(searchParams.get('desde')!) : undefined
  );
  const [fechaHastaFilter, setFechaHastaFilter] = React.useState<Date | undefined>(
    searchParams.get('hasta') ? parseISO(searchParams.get('hasta')!) : undefined
  );
  const currentPage = Number(searchParams.get('page')) || 1;

  const fetchEnvios = React.useCallback(async (page: number) => {
    setIsLoading(true);
    const filters = {
      searchTerm: searchTerm || undefined,
      estado: estadoFilter || undefined,
      fechaDesde: fechaDesdeFilter ? format(fechaDesdeFilter, 'yyyy-MM-dd') : undefined,
      fechaHasta: fechaHastaFilter ? format(fechaHastaFilter, 'yyyy-MM-dd') : undefined,
    };
    const { envios: data, count, error } = await getEnviosAction(filters, page, ITEMS_PER_PAGE);
    ...
    setIsLoading(false);
  }, [toast, searchTerm, estadoFilter, fechaDesdeFilter, fechaHastaFilter]);
  ...
  }, [searchTerm, estadoFilter, fechaDesdeFilter, fechaHastaFilter, currentPage, fetchEnvios, router, pathname, searchParams]);
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" className={cn("w-full justify-start text-left font-normal", !fechaDesdeFilter && "text-muted-foreground")}>
                  ...
                  {fechaDesdeFilter ? format(fechaDesdeFilter, "PPP", { locale: es }) : <span>Fecha Desde</span>}
                </Button>
              </PopoverTrigger>
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" className={cn("w-full justify-start text-left font-normal", !fechaHastaFilter && "text-muted-foreground")}>
                  <CalendarIcon className="mr-2 h-4 w-4" />
                  {fechaHastaFilter ? format(fechaHastaFilter, "PPP", { locale: es }) : <span>Fecha Hasta</span>}
                </Button>
              </PopoverTrigger>
